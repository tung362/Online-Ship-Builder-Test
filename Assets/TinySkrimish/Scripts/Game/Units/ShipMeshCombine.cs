...
//Optimizes draw calls for ships with multiple child objects
public class ShipMeshCombine : TungDoesMathForYou
{
    /*Settings*/
    ...
    /*Required Components*/
    private UnitID TheUnitID;

    void Start()
    {
        TheUnitID = GetComponent<UnitID>();
        if (CombineOnStart) CombineMeshes();
    }

    void LateUpdate()
    {
        if (RunOnce) CombineMeshes();
    }
    ...
    void CombineMeshes()
    {
        ...
        for (int materialID = 0; materialID < avaliableMaterials.Count; materialID++)
        {
            ...
            for (int i = 0; i < childFilters.Length; i++)
            {
                ...
                else childFilters[i].GetComponent<ShipChangeColor>().UpdateColor(TheUnitID.ID);
            }
        ...
        for (int i = 0; i < finalCombiners.Count; i++)
        {
            ...
            else
            {
                ...
                chunk.AddComponent<ShipChangeColor>();
                ...
                chunk.GetComponent<ShipChangeColor>().UpdateColor(TheUnitID.ID);
                AllChunks.Add(chunk);
        ...
        finalChunk.AddComponent<ShipChangeColor>();
        ...
        finalChunk.GetComponent<ShipChangeColor>().UpdateColor(TheUnitID.ID);
        AllChunks.Add(finalChunk);
        RunOnce = false;
    }

    public void RequestCombine()
    {
        RunOnce = true;
    }
}
